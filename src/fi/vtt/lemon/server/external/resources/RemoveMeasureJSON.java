 ...
 */
@Path(PATH_REMOVE_MEASURE)
public class RemoveMeasureJSON {
  private final static Logger log = new Logger(RemoveMeasureJSON.class);
  ...
  public Response addMeasure(@HeaderParam("authorization") String authHeader, JSONObject req) {
    ...
    if (registry.check(authHeader)) {
      try {
        stp.removeMeasurement(req.getString(MEASURE_URI), req.getString(PARAM_CONFIG));
