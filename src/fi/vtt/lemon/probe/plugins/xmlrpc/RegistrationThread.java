...
//  private ServerAgent destination;
  ...
  public void start() {
    ...
    t.setDaemon(true);
    t.start();
    log.debug("started registration thread");
  }
  ...
  public synchronized void stop() {
    ...
    log.debug("stopped registration thread");
  }
...
//      log.debug("all probes registered");
  ...
  private synchronized void registerProbeInformation(Probe probe) {
    ...
    while (shouldRun) {
      try {
        ...
        if (probeId < 1) {
          ...
          log.debug("Failed to register probe. Error code:"+probeId+" = "+errorMsg);
        ...
        log.error("Failed to register with server. Waiting "+config.getRetryDelay()+" milliseconds to retry.", e);
        try {
          wait(config.getRetryDelay());
    ...
    log.debug("Registration thread registration loop terminated");
  }
